...
class RandomPlayer(Player):
    ''' A subclass of Player that move randomly
    * A random player will pick a random move from among the legal possibilities.'''
...
class UserPlayer(Player):
    ...
    * A user player will prompt the user to select a move rather than having the computer choose the move.
    '''
    def move(self,current: int, minimum: int, maximum: int, goal: int) -> int:
        steps = input(f'Enter move of steps between {minimum} and {maximum}: ')#f(x,y) = 5+x+y
        # while the user enter invalid input, keep promting user with input request
        while not steps.isdigit() or steps < minimum or steps> maximum:
            if not steps.isdigit():
                print('Please enter a number!')
            else:
                print('Invalid Input!')
            steps = input(f'Enter move of steps between {minimum} and {maximum}: ')
        return int(steps)
...
def make_player(generic_name: str) -> Player:
    ...
    # f(x) = 5+x
    typeOfPlayer = input(f'Enter a type of the player for {generic_name}: **U=User,R=Random,S=Strategic')
    while typeOfPlayer not in ['U','R','S']:
        typeOfPlayer = input(f'Enter a type of the player for {generic_name}: **U=User,R=Random,S=Strategic')
    if typeOfPlayer == 'U':
