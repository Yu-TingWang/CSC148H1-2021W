...
class RecursiveList:
    ...
    def selections(self) -> [RecursiveList]:
        if self.is_empty():
            ...
            for lst in rest_selections:
                selection = RecursiveList([self._first])
                # first option: to select lst._first
                selection._rest = lst.copy()
                select_first.append(selection)
    ...
    def copy(self) -> RecursiveList:
        if self.is_empty():
            ...
            new_lst = RecursiveList([])
            new_lst._first = self._first
            new_lst._rest = self._rest
            return new_lst
